...
impl<'a, 'tcx: 'a> FunctionInstrumenter<'a, 'tcx> {
    ...
    fn func_hash(&self) -> (u64, u64) {
        self.tcx.def_path_hash(self.body.source.def_id()).0.as_value()
    }
}
...
impl<'a, 'tcx: 'a> Visitor<'tcx> for FunctionInstrumenter<'a, 'tcx> {
    ...
    fn visit_statement(&mut self, statement: &Statement<'tcx>, mut location: Location) {
        ...
        let move_fn = self
            .find_instrumentation_def(Symbol::intern("ptr_move"))
            .expect("Could not find pointer move hook");
        let store_fn = self
        ...
        if let StatementKind::Assign(assign) = &statement.kind {
            println!(
                "statement: {:?} | {:?}",
                statement,
                self.tcx.def_path_hash(self.body.source.def_id())
            );
            ...
            if value.ty(&self.body.local_decls, self.tcx).is_integral() {
            ...
            if value.ty(&self.body.local_decls, self.tcx).is_unsafe_ptr() {
                ...
                if let Rvalue::Use(Operand::Copy(p)) = value {
                ...
                } else if let Rvalue::Cast(_, Operand::Copy(p), _) = value {
                    location.statement_index += 1;
                    if p.ty(&self.body.local_decls, self.tcx).ty.is_integral() {
                        self.add_instrumentation(
                            location,
                            ptr_contrive_fn,
                            vec![Operand::Copy(dest)],
                ...
                } else if let Rvalue::Cast(_, Operand::Move(p), _) = value {
                    location.statement_index += 1;
                    if p.ty(&self.body.local_decls, self.tcx).ty.is_integral() {
                        self.add_instrumentation(
                            location,
                            ptr_contrive_fn, // TODO: do copies and moves need to be separate?
                            vec![Operand::Copy(dest)],
                            false,
                            false,
                            EventMetadata {
                ...
                } else if let Rvalue::Use(Operand::Move(p)) = value {
                    location.statement_index += 1;
                    self.add_instrumentation(
                        location,
                        move_fn,
        ...
        match &terminator.kind {
            TerminatorKind::Call {
                ...
                if let ty::FnDef(def_id, _) = func.ty(self.body, self.tcx).kind() {
                    ...
                    println!("visiting func {:?}", fn_name);
                    if HOOK_FUNCTIONS.contains(&fn_name.as_str()) {
                        // the first arg will be the MirLoc, so skip it
                        arg_idx += 1;
                    }
                }
                for arg in args {
                    let dest_func = if let ty::FnDef(def_id, _) = func.ty(self.body, self.tcx).kind() {
                        ...
                        if self.body.local_decls[place.local].ty.is_unsafe_ptr() {
                            ...
                            self.add_instrumentation(
                                ...
                                EventMetadata {
                                    ...
                                    dest_func: dest_func,
                                    // destination: destination.map(|p| to_mir_place(&p.0)),
                                },
                            );
                        }
                    }
                ...
                if let ty::FnDef(def_id, _) = func.ty(self.body, self.tcx).kind() {
                    ...
                    if HOOK_FUNCTIONS.contains(&fn_name.as_str()) {
                        ...
                        println!(
                            "HOOK caller: ({:?}), callee: ({:?})",
                            self.tcx.def_path_hash(*def_id),
                            self.tcx.def_path_hash(func_def_id)
                        );
                        self.add_instrumentation(
                            location,
                            func_def_id,
